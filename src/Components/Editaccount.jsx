...
const EditAccount = () => {
    const { isAuthenticated, csrfToken } = useContext(AuthContext); // Use AuthContext for authentication
    ...
    const handleUpdateAccount = async (event) => {
        ...
        try {
            ...
            if (profileImage) {
                formData.append('profileImage', profileImage); // Add the profile image if it was changed
            }
            if (password) {
                formData.append('password', password); // Add new password if provided
            }
